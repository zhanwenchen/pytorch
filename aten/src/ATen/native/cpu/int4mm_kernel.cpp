...
static bool can_use_kleidiai(
  ...
  if (cpuinfo_has_arm_neon_dot()) {
    ...
    // implementation.
    if ((block_size == K && scales_zeros.dtype() == kFloat) ||
        ((block_size < K && !(block_size % 32) && !(K % block_size)) &&
         scales_zeros.dtype() == kBFloat16)) {
      ret = true;
    }
  }
  return ret;
...
static void ref_dyn_quant_matmul_4bit_channelwise_kernel(
  ...
  uint8_t* lhs_qa8dx = new uint8_t[input_size_8bit];
...
static void ref_dyn_quant_matmul_4bit_groupwise_kernel(
        ...
        for (size_t row_idx = 0; row_idx < m; ++row_idx) {
          ...
          *((int32_t*)(dst_ptr)) = -nudged_zero_point0;
          ...
          for (size_t k_idx = 0; k_idx < k; ++k_idx) {
            ...
            int32_t v0_s32 = (int32_t)(round(src0_0 * scale0));
            v0_s32 = std::max(
                std::min(v0_s32 + nudged_zero_point0, INT8_MAX), INT8_MIN);
            dst_ptr[0] = (int8_t)v0_s32;
            dst_ptr += sizeof(int8_t);
          }
        }
  ...
  int8_t* lhs_qa8dx =
      new int8_t[m * (k + sizeof(float) + sizeof(int32_t))]; // Allocate for LHS
