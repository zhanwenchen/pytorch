...
class UnflattenedModule(torch.nn.Module):
    def __init__(
        ...
        id_to_buffer: Dict[int, Tuple[torch.nn.Parameter, bool]] = {}  # handle weight-sharing
        ...
        for name, tensor in state_dict.items():
            ...
            if is_buffer:
                if id(tensor) not in id_to_buffer:  # this is completely unused (not weight-sharing)
                    id_to_buffer[id(tensor)] = (tensor, True)  # assign to respect original model
                _assign_attr(
        ...
        # this loop adds aliased/unused params, buffers (not in graph signature)
